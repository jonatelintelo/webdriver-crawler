...
def crawl(options, domains, noop, accept):
    ...
    for domain in domains:
        ...
        errors = dict(zip(['page_load_timeout','dns','consent_click'],[timeout_error,dns_error,None]))
        ...
        visit['request_date'], \
        visit['x_domain_redirects'] = utils.get_header_data(driver, tracker_dict, tracker_dict_values)
       
        visit['cookies'] = driver.get_cookies()

        visit['third_party_request_domains'] = utils.get_third_party_data(domain, visit['request_headers'])
        visit['third_party_response_domains'] = utils.get_third_party_data(domain, visit['response_headers'])
